...
<body>
	<h1>회원가입 페이지</h1>
	<hr>
	<form action="signup" method="post" onsubmit="return inputCheck()">
	...
	<script>
		...
		const div_nickname_result = document.createElement("div");
		...
		function nicknameCheck() {
			...
			xhr.onload = function () {
				let checkResult = xhr.responseText;
				if (checkResult === "1") {
		...
		function inputCheck() {
	        if (txt_nickname.value.length == 0) {
	            alert("닉네임을 입력하세요.");
	            txt_nickname.focus();
	            return false;
	        }
